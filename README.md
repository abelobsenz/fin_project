...
ivdyn pull-options-symbol --data-root data --symbol SPY --start-date 2024-07-01 --end-date 2024-12-31
ivdyn pull-massive --data-root data --symbol SPY --start-date 2024-07-01 --end-date 2024-12-31
ivdyn build-dataset --data-root data --out-dir outputs/datasets/dataset
ivdyn build-dataset --data-root data --out-dir outputs/datasets/dataset_2024h2 --plugin massive_flatfile_aggs --start-date 2024-07-01 --end-date 2024-12-31
ivdyn train --dataset outputs/datasets/dataset/dataset.npz --out-dir outputs/runs
ivdyn evaluate --run-dir outputs/runs/<RUN_ID> --dataset outputs/datasets/dataset/dataset.npz
ivdyn backtest --run-dir outputs/runs/<RUN_ID> --dataset outputs/datasets/dataset/dataset.npz
...
- `data/symbols/SPY/options/raw/*.parquet` and `*.metadata.json` (Massive-derived chain snapshots)
- `data/symbols/SPY/underlying/spy_eod.parquet` (underlying EOD prices)
- Full-exchange OPRA source: `data/options_source/us_options_opra/day_aggs_v1/YYYY/MM/*.csv.gz`
- Symbol raw options (from source extraction): `data/symbols/SPY/options/raw/*.parquet`

`pull-underlying-massive` fetches true underlying daily bars into
`data/symbols/<SYMBOL>/underlying/<symbol>_eod.parquet`.

`pull-massive` uses the Massive REST snapshot endpoint and writes daily files to
`data/symbols/<SYMBOL>/options/raw`.
Set `MASSIVE_API_KEY` (or pass `--api-key`).

`pull-flatfiles` uses Massive's S3-compatible flatfiles endpoint and writes full OPRA day files under:
`data/options_source/us_options_opra/day_aggs_v1/YYYY/MM/*.csv.gz`.

`pull-options-symbol` extracts one symbol from `data/options_source` and writes canonical daily files to:
